...
def upload_to_threads(video_path, text):
    ...
    try:
        ...
        for attempt in range(max_attempts):
            try:
                print(f"[threads] 📤 Step 1: Uploading to temporary hosting (Attempt {attempt+1}/{max_attempts})...")
                ...
                if temp_response.status_code == 200:
                    ...
                    if temp_url:
                        ...
                        break
                print(f"[threads] ⚠️ Attempt {attempt+1} failed: {temp_response.status_code}")
            except Exception as e:
                print(f"[threads] ⚠️ Attempt {attempt+1} failed: {e}")
